...
const iw = document.querySelector('iframe#g_iframe').contentWindow;
iw.addEventListener("load", () => {
    const id = new URLSearchParams(iw.location.search).get("id");
    const name = iw.document.title.split(" - ").slice(0, 2).join(" - ") + ".mp3";
    const isVIP = iw.document.querySelector('i.lab.u-icn.u-icn-98');
    ...
    styles.textContent = `#cmdl {
        ...
        text-align: center;
        white-space: nowrap;
        background: linear-gradient(135deg, steelblue);
